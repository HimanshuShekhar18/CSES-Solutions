...
void solve(){
  ...
  while(!pq.empty()){
      ...
      for(auto it: graph[node]){
        ...
        if(leftout==1) continue;

        if(distance<dist[vertex][leftout+1]){
          dist[vertex][leftout+1]=distance+dis/2;
          pq.push({vertex,leftout+1,distance+dis/2});
        }
