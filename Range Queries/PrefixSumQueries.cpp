...
void build(int id = 1, int l = 0, int r = n){
    if(r-l==1){
        seg[id].sum = seg[id].prefix_sum = arr[l];
        return;
    }
...
void modify(int pos, int val, int id = 1, int l = 0, int r = n){
    if(r-l==1) {
        seg[id].sum = seg[id].prefix_sum = val;
        return;
     }
...
st query(int x, int y, int id = 1, int l = 0, int r = n){
    if(l>=x && r<=y) {
    return {seg[id].sum,seg[id].prefix_sum}; // When [L,R) is entirely inside [x,y)
    }
    if(r<=x || l>=y) return {0,0};   // When [L,R) is entirely outside [x,y)
...
void solve(){
       ...
       while(q--){
        in3(a,u,v);
        if(a==1) modify(u-1,v);
        else cout << max(0LL,query(u-1,v).prefix_sum) << endl;
       }
